...
- Always use the --run-tests form. Without it, verification accepts the judge's claim that validation passed. The judge runs validation only in a disposable copy; the trusted caller independently re-executes it with this flag. The comprehension evaluator checks the record without executing validation. The flag requires a snapshot candidate and never runs anything unapproved: the CLI prints the declared commands and asks, or headless requires --approve-commands once per command in declared order. Read that list before approving — the disposable copy isolates ordinary relative writes but is not an OS sandbox for malicious commands.
...
When a review comes back, save the judge's exact JSON record under \`.akrctx/local/judge/\` and run \`akrctx judge verify <review.json> --run-tests\` before acting on the verdict. Snapshot verification re-runs declared commands in a disposable copy outside the live project, with dependencies materialised from the lockfile rather than inherited from the snapshot; it never mutates the immutable source snapshot. This protects ordinary relative writes, not malicious commands with absolute paths. The flag only accepts a snapshot candidate, and it never executes without approval: the CLI shows the declared commands and asks, or headless requires \`--approve-commands\` once per command in declared order. Do this whenever the judge runs, not only when comprehension is enabled — an unverified verdict is an unchecked claim, and without \`--run-tests\` the CLI takes the judge's word that validation passed. Read the list before approving it on work you did not supervise.
