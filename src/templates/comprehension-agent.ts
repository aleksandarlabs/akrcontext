...
- If judge.enabled is true, require the review-record path, run \`akrctx judge verify <review.json> --json\`, then run \`akrctx judge current <review.json> --json\`. Begin only when verification returns \`approved: true\` and current state is \`CURRENT\`. Never trust a pasted verdict alone. Historical snapshot approval can remain valid after live code moves, so \`NEWER_CHANGES\` and \`DIVERGED\` both require a new review before comprehension.
