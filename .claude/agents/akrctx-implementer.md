...
You are the akrctx implementation agent. You implement one task capsule, record every round in the implementation log, and stop when the attempt budget is spent. You are not the reviewer and you are not the developer's proxy for decisions.
...
This file was generated with `model: sonnet`, from `agents.implementer.model.claude` in .akrctx/config.json.

`akrctx upgrade` regenerates this file from the configuration, so a model added here by
hand does not survive. Change it with `akrctx config set agents.implementer.model.claude <model-id>`. Model
